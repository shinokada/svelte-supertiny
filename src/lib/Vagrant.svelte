<script lang="ts">
  import { getContext } from 'svelte';
  interface CtxType {
    fill?: string;
    size?: string;
    role?: string;
  }
  const ctx: CtxType = getContext('iconCtx') ?? {};
  interface Props {
    size?: string;
    role?: string;
    ariaLabel?: string;
    class?: string;
    fill?: string;
  }
  let {
    size = ctx.size || '24',
    role = ctx.role || 'img',
    ariaLabel = 'Vagrant',
    fill = ctx.fill || '#fff',
    class: classname,
    ...restProps
  }: Props = $props();
</script>

<svg
  xmlns="http://www.w3.org/2000/svg"
  width={size}
  height={size}
  class={classname}
  {...restProps}
  aria-label="Vagrant"
  {role}
  viewBox="0 0 512 512"
  ><path d="m0 0H512V512H0" {fill} /><path
    d="m418 114.3-73.65 42.84v25.88l-58.94 127.1L256 330.46v93.04l63.14-36.4 98.89-242.12zm-206.2 46.78V131.9l-.3-.18-43.9 25.4v25.9l59 132.7L256 301.16v-37z"
    fill="#1159cc"
  /><path
    d="M167.63 183v-25.93l44.19-25.44-73.66-43.15-44.19 25.8v31.6l99.45 241.5 62.58 36.1v-90.7l29.43-17.1-.35-.2 59.29-132.46v-25.9l73.66-42.8-44.2-25.8-73.6 43.1h-.05v29.48L256 264.16v34.45l-29.47 17.1z"
    fill="#127eff"
  /></svg
>

<!--
@component
[Go to docs](https://svelte-supertiny.codewithshin.com/)
## Props
@props: size?:  string; = ctx.size || '24';
@props:role?:  string; = ctx.role || 'img';
@props:ariaLabel?:  string; = 'Vagrant';
@props:class?: string;
@props:fill?:  string; = ctx.fill || '#fff';
-->
